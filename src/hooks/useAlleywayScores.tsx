...
export const useAlleywayScores = () => {
  const { user } = useAuth();
  ...
  const fetchScores = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to fetch alleyway scores:", err);
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const saveScore = useCallback(
    async (score: number, themeShifts: number) => {
      ...
      try {
        const { error } = await supabase.from("alleyway_scores").insert({
          user_id: user.id,
          score,
        ...
        console.error("Failed to save alleyway score:", err);
