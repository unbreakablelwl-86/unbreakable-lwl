...
export const useWordChainScores = () => {
  const { user } = useAuth();
  ...
  const fetchScores = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to fetch word chain scores:", err);
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const saveScore = useCallback(
    async (score: number, wordsFound: number, longestWord: string, maxChain: number) => {
      if (!user) return;
      try {
        const { error } = await supabase.from("word_chain_scores" as any).insert({
          user_id: user.id,
          score,
        ...
        console.error("Failed to save word chain score:", err);
