...
export const useMentalMathsScores = () => {
  const { user } = useAuth();
  ...
  const fetchScores = useCallback(async () => {
    ...
    try {
      const { data: scores, error } = await supabase
        .from("mental_maths_scores" as any)
        .select("*")
      ...
      console.error("Failed to fetch mental maths scores:", err);
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const saveScore = useCallback(
    async (score: number, solved: number, accuracy: number) => {
      if (!user) return;

      try {
        const { error } = await supabase.from("mental_maths_scores" as any).insert({
          user_id: user.id,
          score,
          solved,
          accuracy,
        ...
        console.error("Failed to save mental maths score:", err);
