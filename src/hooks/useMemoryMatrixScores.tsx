...
export const useMemoryMatrixScores = () => {
  const { user } = useAuth();
  ...
  const fetchScores = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to fetch memory matrix scores:", err);
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const saveScore = useCallback(
    async (score: number, maxLevel: number) => {
      if (!user) return;

      try {
        const { error } = await supabase.from("memory_matrix_scores" as any).insert({
          user_id: user.id,
          score,
          max_level: maxLevel,
        } as any);
        ...
        console.error("Failed to save memory matrix score:", err);
