        ...
        {!user ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {features.map(f => (
                <div key={f.title} className="rounded-xl border border-gray-800 bg-[#111] p-4 text-center space-y-2">
                  ...
                  <h3 className="font-display text-[11px] tracking-wider text-white">{f.title}</h3>
            ...
            <div className="rounded-xl border border-[#FF5500]/20 bg-[#111] p-8 text-center">
              ...
              <button onClick={() => setShowAuthModal(true)} className="px-5 py-2.5 rounded-xl bg-[#FF5500] text-white font-display text-xs tracking-wider hover:bg-[#FF5500]/80 transition-colors">
                SIGN IN
              </button>
            </div>
          </div>
            ...
            {myPendingRequest ? (
              ...
              <div className="rounded-xl border border-gray-800 bg-[#111] p-6 text-center">
                ...
                <button
                  ...
                  disabled={requesting}
                  className="flex items-center gap-2 mx-auto px-5 py-2.5 rounded-xl bg-[#FF5500] text-white font-display text-xs tracking-wider hover:bg-[#FF5500]/80 transition-colors disabled:opacity-50"
                >
            ...
            <div className="rounded-xl border border-[#FF5500]/20 bg-[#FF5500]/5 p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Avatar className="h-12 w-12 border-2 border-[#FF5500]/30">
                    <AvatarImage src={myCoach.coach_profile?.avatar_url || undefined} />
                    <AvatarFallback className="font-display bg-[#FF5500]/20 text-[#FF5500]">
                      {(myCoach.coach_profile?.display_name || '?')[0].toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    <p className="text-[10px] font-display tracking-wider text-gray-500">YOUR COACH</p>
                    <p className="font-display text-sm tracking-wide text-white">{myCoach.coach_profile?.display_name || 'Coach'}</p>
                    <p className="text-[11px] text-gray-500">@{myCoach.coach_profile?.username || 'coach'}</p>
                  </div>
                </div>
                <Link to={`/inbox?compose=1&to=${myCoach.coach_id}`}>
                  <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#FF5500]/30 text-[#FF5500] font-display text-[11px] tracking-wider hover:bg-[#FF5500]/10 transition-colors">
                ...
                {activeCheckIn ? (
                  ...
                  checkIns.map(ci => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-display text-sm tracking-wide text-white">CHECK-IN #{ci.check_in_number}</p>
                      ...
                      <Link key={p.id} to="/programming/my-programmes">
                        <div className="rounded-xl border border-gray-800 bg-[#111] p-3 flex items-center gap-3 hover:border-[#FF5500]/20 transition-colors">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="font-display text-sm tracking-wide text-white truncate">{p.name}</p>
                            <p className="text-[11px] text-gray-500">Power Programme</p>
                          </div>
                          <span className="text-[9px] font-display tracking-wider px-2 py-0.5 rounded-full border border-[#FF5500]/20 text-[#FF5500]">ACTIVE</span>
                        </div>
                      </Link>
                    ))}
                    {activeCardio.map(p => (
                      <Link key={p.id} to="/tracker/my-programmes">
                        <div className="rounded-xl border border-gray-800 bg-[#111] p-3 flex items-center gap-3 hover:border-[#FF5500]/20 transition-colors">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="font-display text-sm tracking-wide text-white truncate">{p.name}</p>
                            <p className="text-[11px] text-gray-500">Movement Programme</p>
                          </div>
                          <span className="text-[9px] font-display tracking-wider px-2 py-0.5 rounded-full border border-[#FF5500]/20 text-[#FF5500]">ACTIVE</span>
                        </div>
                      </Link>
                    ...
                    {activeMeals.map(p => (
                      <Link key={p.id} to="/fuel/planning">
                        <div className="rounded-xl border border-gray-800 bg-[#111] p-3 flex items-center gap-3 hover:border-[#FF5500]/20 transition-colors">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="font-display text-sm tracking-wide text-white truncate">{p.name}</p>
                            <p className="text-[11px] text-gray-500">Meal Plan</p>
                          </div>
                          <span className="text-[9px] font-display tracking-wider px-2 py-0.5 rounded-full border border-[#FF5500]/20 text-[#FF5500]">ACTIVE</span>
                        </div>
                      </Link>
