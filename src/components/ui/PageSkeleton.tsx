    ...
    <div className="p-4 space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex gap-3 overflow-hidden">
        {[0, 1, 2].map(i => (
          <div key={i} className="flex-shrink-0 w-36">
            <Skeleton className="h-36 w-36 rounded-lg bg-white/[0.04]" />
            <Skeleton className="h-3 w-24 mt-2 bg-white/[0.06]" />
          </div>
        ))}
      </div>
      {/* Tracks */}
      {[0, 1, 2, 3].map(i => (
        <div key={i} className="flex items-center gap-3 py-2">
          <Skeleton className="h-10 w-10 rounded bg-white/[0.04]" />
      ...
      {[0, 1, 2].map(i => (
        <div key={i} className={`flex ${i % 2 === 0 ? 'justify-start' : 'justify-end'}`}>
          <div className={`rounded-xl p-3 space-y-2 ${i % 2 === 0 ? 'w-3/4' : 'w-2/3'}`}>
            ...
            <Skeleton className="h-3 w-4/5 bg-white/[0.04]" />
            {i % 2 === 0 && <Skeleton className="h-3 w-2/3 bg-white/[0.04]" />}
          </div>
        </div>
      ))}
      {/* Input */}
