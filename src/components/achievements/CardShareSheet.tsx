...
export function CardShareSheet({
  ...
  // Share via native share sheet (2x image)
  const handleNativeShare = useCallback(async () => {
    ...
    try {
      ...
      const file = new File(
        [blob],
        `unbreakable-${tier}-${(card.title || 'card').replace(/\s+/g, '-').toLowerCase()}.png`,
        { type: 'image/png' },
      );

      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        await navigator.share({
          title: `Unbreakable ${cfg.label} Card`,
          text: caption,
          files: [file],
        });
  ...
  }, [card, cardSystem, tier, cfg.label, caption, toast, onOpenChange]);
