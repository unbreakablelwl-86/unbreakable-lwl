...
interface CardioNextSessionPreviewProps {
  programId: string;
  currentWeek: number;
  currentDay: number;
}

// Mirrors programming/NextSessionPreview.tsx (Power) so the Movement library
// card shows the same progress bar + "next session" breakdown when expanded,
// instead of jumping straight to the full programme display with no sense
// of what's actually coming up next or how far through the plan you are.
export function CardioNextSessionPreview({ programId, currentWeek, currentDay }: CardioNextSessionPreviewProps) {
  const { planners, isLoading } = useCardioSessionPlanners(programId);
  ...
  const segments: any[] = nextSession.planned_session?.mainSession || [];
  ...
  return (
    <div className="space-y-3 mb-4">
      ...
      <Card className="p-4 border-primary/25 bg-gradient-to-br from-primary/5 to-transparent shadow-[0_0_15px_hsl(var(--primary)/0.08)]">
        ...
        {(nextSession.duration_minutes || nextSession.distance_km) && (
          <div className="flex items-center gap-4 mb-2 text-xs text-muted-foreground">
            {nextSession.duration_minutes && (
              <span className="flex items-center gap-1"><Clock className="w-3 h-3 text-primary/50" /> {nextSession.duration_minutes} min</span>
            )}
            {nextSession.distance_km && (
              <span className="flex items-center gap-1"><Target className="w-3 h-3 text-primary/50" /> {nextSession.distance_km} km</span>
            )}
          </div>
        )}

        {/* Segment breakdown — the cardio equivalent of Power's exercise list */}
        {segments.length > 0 && (
            ...
            {segments.map((segment, i) => (
              <div key={i} className="flex items-center gap-2.5 py-1">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-foreground truncate">{segment.segment}</p>
                  {segment.notes && (
                    <p className="text-[10px] text-muted-foreground truncate">{segment.notes}</p>
                  )}
                </div>
                {segment.duration && (
