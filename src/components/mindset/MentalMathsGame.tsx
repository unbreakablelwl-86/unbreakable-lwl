...
const NAMED_STAGES = [
  { threshold: 0,   name: "WARM UP",    label: "STAGE 1",  timePerQ: 12,  maxNum: 10,  multMax: 3,  divMax: 3,  ops: ["+", "-", "×", "÷"] as string[] },
  { threshold: 10,  name: "MOVING",     label: "STAGE 2",  timePerQ: 11,  maxNum: 12,  multMax: 4,  divMax: 4,  ops: ["+", "-", "×", "÷"] },
  { threshold: 25,  name: "PICKING UP", label: "STAGE 3",  timePerQ: 10,  maxNum: 15,  multMax: 5,  divMax: 5,  ops: ["+", "-", "×", "÷"] },
  { threshold: 45,  name: "SHARPER",    label: "STAGE 4",  timePerQ: 9,   maxNum: 20,  multMax: 6,  divMax: 6,  ops: ["+", "-", "×", "÷"] },
  { threshold: 70,  name: "LOCKED IN",  label: "STAGE 5",  timePerQ: 8,   maxNum: 25,  multMax: 7,  divMax: 7,  ops: ["+", "-", "×", "÷"] },
  { threshold: 100, name: "ON FIRE",    label: "STAGE 6",  timePerQ: 7,   maxNum: 35,  multMax: 8,  divMax: 8,  ops: ["+", "-", "×", "÷"] },
  { threshold: 140, name: "UNTOUCHABLE",label: "STAGE 7",  timePerQ: 6,   maxNum: 50,  multMax: 10, divMax: 10, ops: ["+", "-", "×", "÷"] },
  { threshold: 190, name: "GODSPEED",   label: "STAGE 8",  timePerQ: 5.5, maxNum: 75,  multMax: 12, divMax: 10, ops: ["+", "-", "×", "÷"] },
  { threshold: 250, name: "IMMORTAL",   label: "STAGE 9",  timePerQ: 5,   maxNum: 100, multMax: 15, divMax: 12, ops: ["+", "-", "×", "÷"] },
  { threshold: 350, name: "UNBREAKABLE",label: "STAGE 10", timePerQ: 4.5, maxNum: 150, multMax: 20, divMax: 15, ops: ["+", "-", "×", "÷"] },
];
...
const generateQuestion = (stage: typeof NAMED_STAGES[0]): Question => {
  ...
  if (op === "+") {
    ...
    a = Math.floor(Math.random() * (stage.multMax - 1)) + 2;
    b = Math.floor(Math.random() * (stage.multMax - 1)) + 2;
    answer = a * b;
  } else {
    b = Math.floor(Math.random() * (stage.divMax - 1)) + 2;
    answer = Math.floor(Math.random() * (stage.divMax - 1)) + 1;
    a = b * answer;
  }
