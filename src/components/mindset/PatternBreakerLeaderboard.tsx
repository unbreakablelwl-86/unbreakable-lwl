...
interface PatternBreakerLeaderboardProps {
  scores: ScoreEntry[];
...
export const PatternBreakerLeaderboard = ({ scores, userBest, onClose, onRefetch }: PatternBreakerLeaderboardProps) => {
  ...
  return (
    <div className="w-full max-w-lg mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <h2 className="font-display text-2xl text-primary tracking-wide">LEADERBOARD</h2>
      ...
      <div className="space-y-2">
        {scores.length === 0 ? (
          ...
          scores.map((entry, index) => (
            ...
            >
              <Card className="border-border hover:border-primary/30 transition-all p-3 bg-card">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-right">
                    ...
                    {entry.max_sequence > 0 && (
                      <p className="text-[10px] text-muted-foreground">{entry.max_sequence} seq</p>
                    )}
                  </div>
                </div>
              </Card>
