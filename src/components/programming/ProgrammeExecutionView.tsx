...
export function ProgrammeExecutionView({ program, onClose }: ProgrammeExecutionViewProps) {
  ...
  const { updateProgress, setAutoTrack, startProgrammeExecution } = useTrainingPrograms();
  ...
  // generated..." forever (JJ, Sept 2026 — found on a live active programme
  // during the full-site audit: that message described something that was
  // never actually happening. startProgrammeExecution normally generates
  // planners at activation time, but if that step failed after is_active was
  // already set — or the programme was activated by some other path — the
  // programme is left active with zero planners and no code ever retries.
  // startProgrammeExecution is safe to call again here: it only inserts
  // planners when none exist yet, so this is a no-op for every normal
  // programme and just fixes the ones that got stuck. Guarded to fire once
  // per mount so a genuine, unrelated generation failure doesn't loop.
  const hasAttemptedPlannerRecovery = useRef(false);
  useEffect(() => {
    if (plannersLoading) return;
    ...
    startProgrammeExecution.mutate(program.id);
  }, [plannersLoading, planners, program.id, startProgrammeExecution]);
