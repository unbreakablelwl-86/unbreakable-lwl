...
export function StatusCard({ post, onKudos, onDelete, onToggleComments, onUpdatePost, onOpenStoryEditor }: StatusCardProps) {
  ...
  // Music-on-post ("add music") was removed — it was never wired into the
  // composer, the DB media_type column never allowed 'audio', and no post
  // ever carried an audio item, so this is a no-op filter kept only so the
  // rest of the component doesn't need to change shape.
  const visualMedia = post.media_items || [];
  const hasMedia = visualMedia.length > 0 || post.image_url || post.video_url;
