...
serve(async (req) => {
  ...
  try {
    ...
    const rateLimited = await checkRateLimit(svcClient, tokenUserId, 'generate-program');
    if (rateLimited) {
    ...
    const tokenGuard = await requireToken(svcClient, tokenUserId, 'generate-program', 'programme_build');
    if (tokenGuard.error) {
