...
serve(async (req) => {
  ...
  try {
    ...
    const rateLimited = await checkRateLimit(svcClient, tokenUserId, 'analyze-movement');
    if (rateLimited) {
    ...
    const tokenGuard = await requireToken(svcClient, tokenUserId, 'analyze-movement', 'workout_feedback');
    if (tokenGuard.error) {
