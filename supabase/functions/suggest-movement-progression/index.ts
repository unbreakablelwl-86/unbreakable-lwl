...
serve(async (req) => {
  ...
  try {
    ...
    const rateLimited = await checkRateLimit(svcClient, tokenUser.id, 'suggest-movement-progression');
    if (rateLimited) {
    ...
    const tokenGuard = await requireToken(svcClient, tokenUser.id, 'suggest-movement-progression', 'progression_tip');
    if (tokenGuard.error) {
